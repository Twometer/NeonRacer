...
public abstract class AbstractCarPhysics implements EntityPhysics {
    ...
    protected RevoluteJoint flJoint;
    protected RevoluteJoint frJoint;
    Vec2 relativeVelocity;
    private Vec2 currentDrag = new Vec2(0, 0);
    private Body body;

    AbstractCarPhysics(GameContext gameContext, CarBody carBody, Tire[] tires, RevoluteJoint flJoint, RevoluteJoint frJoint, float dragCoefficient) {
    ...
    public void update(boolean driving, boolean braking) {
        if (!driving && (body.getLinearVelocity().length() < (tires[Tire.BACK_LEFT].getCurrentRelativeFriction().length() / gameContext.getTimer().getTicksPerSecond()))) {  //Killing small velocities when not driving
            ...
            currentDrag = getVelocity().mul(-dragCoefficient * getVelocity().length());             //Calculating air drag force on the car
    ...
    public RevoluteJoint getFlJoint() {
        return flJoint;
    }
}
